...
plot()
for N in [21,41,61]
    local its, w = optimal_SOR_parameter_test(1.5:0.001:1.99, N)
    println(w)
...
for (i,N) in enumerate([21, 61, 101, 201])
    ...
    local w=optimal_SOR_parameter_asym(N)
...
for n=1:l
    ...
    uf = SOR_method_B(N, w; maxit=2N^2, tol=1e-12)
    display(size(uf))
    u_subgrid = uf[1:2^n:end, 1:2^n:end]
    grid_norms[n] = norm(u_subgrid - prev_u_subgrid)
    global prev_u_subgrid = u_subgrid
end
hv = 2 ./ (Nv .- 1)
